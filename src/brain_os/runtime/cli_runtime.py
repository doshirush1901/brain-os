...
Composition root — lives in ``brain_os.runtime`` (not ``brain_os.interfaces``). Typer
sub-apps under ``interfaces/commands/`` and other layers import wiring here
**without** importing ``brain_os.interfaces.cli`` (avoids circular imports once
``cli`` mounts those command modules).
...
def _run(coro: Any) -> Any:
    ...
    from brain_os.runtime.cli_shutdown import (
        patch_aiosqlite_worker_threads_daemon,
        release_cli_non_daemon_threads,
    )

    # Before any aiosqlite connect: workers must be daemon so CLI exit cannot hang.
    patch_aiosqlite_worker_threads_daemon()
    try:
    ...
    try:
        ...
        # Belt-and-suspenders: connections opened before the patch (or other
        # non-daemon leftovers) must not block returning to the shell.
        release_cli_non_daemon_threads(join_timeout_s=0.5)


def _configure_logging(verbose: bool = False, *, dream_progress: bool = False) -> None:
    """Console + rotating ``data/logs/ira.log`` / ``errors.log`` (shared bootstrap)."""
    from brain_os.logging_setup import setup_ira_logging
    from brain_os.runtime.cli_shutdown import patch_aiosqlite_worker_threads_daemon

    setup_ira_logging(verbose=verbose, dream_progress=dream_progress, console=True)
    # Early patch so pantheon/pipeline bootstrap cannot spawn sticky workers.
    patch_aiosqlite_worker_threads_daemon()
...
def _build_pantheon() -> tuple[Any, dict[str, Any]]:
    ...
    retriever = UnifiedRetriever(qdrant=qdrant, graph=graph, long_term=long_term)
    bus = MessageBus()
    ...
    except Exception as e:  # noqa: BLE001 — optional CLI service; absence degrades gracefully
...
async def _build_pipeline(
    ...
    *shared_services* is the dict returned by ``_build_pantheon()`` and
    ...
    async def _safe_init(name: str, coro: Any) -> None:
        ...
        except (ConfigurationError, Exception):  # noqa: BLE001 — optional CLI service; absence degrades gracefully
    ...
    except (TimeoutError, Exception):  # noqa: BLE001 — slow/failed table creation must not block the CLI
        logger.warning("SensorySystem table creation slow/failed — continuing", exc_info=True)

    long_term = shared_services.get(SK.LONG_TERM_MEMORY)
    ...
    procedural_memory = build_procedural_memory()
    ...
    except Exception:  # noqa: BLE001 — optional CLI service; absence degrades gracefully
    ...
    except (BrainOSError, Exception):  # noqa: BLE001 — startup validation failure continues in degraded mode
    ...
    try:
        ...
        if redis_cache.available:
            ...
            get_mem0_search_cache().set_redis_cache(redis_cache)
    except (ConfigurationError, Exception):  # noqa: BLE001 — Redis absence falls back to in-memory pipeline state
...
def _build_email_processor(
    ...
    # L3.3c: composition root may import the concrete EmailProcessor permanently.
    # It only constructs and injects under SK.EMAIL_PROCESSOR (and as SystemNotifier /
    # MailReader / DraftSender). Do not peel this into a protocol — that would push
    # construction into services/systems and recreate inversion edges.
...
def resolve_mail_reader_if_needed(enabled: bool) -> Any | None:
    """L3.3c: build EmailProcessor as MailReader when mailbox enrichment is on.

    Composition-root helper for interfaces CLI/MCP — keeps systems free of
    ``EmailProcessor`` construction.
    """
    if not enabled:
        return None
    pantheon, shared = _build_pantheon()
    digestive = _build_digestive(shared)
    return _build_email_processor(pantheon, digestive, shared)
