...
L3.2: relocated from ``brain_os.interfaces.email_processor_constants``.
``email_processor.py`` may still re-export the externally-imported subset
...
# Gmail practical limit is ~25MB; operator review packs stay under 10MB.
...
def _build_mime_outbound(
    ...
    *,
    ...
    """Build a RFC 822 message, optionally with file attachments and HTML alternative."""
    if attachment_paths:
        total = 0
        ...
        if total > MAX_OUTBOUND_ATTACHMENT_BYTES:
            raise BrainOSError(
                ...
                f"{MAX_OUTBOUND_ATTACHMENT_BYTES} byte guard (<10MB)"
            )
    if html_body and not attachment_paths:
