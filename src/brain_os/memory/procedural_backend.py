...
class ProceduralMemoryBackend(ABC):
    ...
    @abstractmethod
    async def insert_procedure(self, trigger_pattern: str, steps: str, last_used: str) -> int: ...
...
class SqliteProceduralMemoryBackend(ProceduralMemoryBackend):
    ...
    async def insert_procedure(self, trigger_pattern: str, steps: str, last_used: str) -> int:
        ...
        await self._db.execute(
            ...
            VALUES (?, ?, 1.0, 1, ?)
            """,
            (trigger_pattern, steps, last_used),
        )
        await self._db.commit()
    ...
    async def list_rows(
        ...
        cursor = await self._db.execute(
            f"SELECT * FROM procedures ORDER BY {order} LIMIT ?",
            (limit,),
        )
        rows = await cursor.fetchall()
        await cursor.close()
...
class PgProceduralMemoryBackend(ProceduralMemoryBackend):
    ...
    async def insert_procedure(
        self, trigger_pattern: str, steps: str, last_used: str, *, row_id: int | None = None
    ) -> int:
        return await self._repo.insert(trigger_pattern, steps, 1.0, 1, last_used, row_id=row_id)
...
class DualWriteProceduralMemoryBackend(ProceduralMemoryBackend):
    ...
    async def insert_procedure(self, trigger_pattern: str, steps: str, last_used: str) -> int:
        row_id = await self._sqlite.insert_procedure(trigger_pattern, steps, last_used)
        try:
            await self._pg.insert_procedure(trigger_pattern, steps, last_used, row_id=row_id)
        except Exception:
